...
-- romaneio_por_item (Ravex / conferência — habilitar RLS + policies completas)
do $$
begin
  if to_regclass('public.romaneio_por_item') is not null then
    execute 'alter table public.romaneio_por_item enable row level security';
    execute 'drop policy if exists "Permitir SELECT em romaneio_por_item" on public.romaneio_por_item';
    execute 'drop policy if exists "Permitir INSERT em romaneio_por_item" on public.romaneio_por_item';
    execute 'drop policy if exists "Permitir UPDATE em romaneio_por_item" on public.romaneio_por_item';
    execute 'drop policy if exists "Permitir DELETE em romaneio_por_item" on public.romaneio_por_item';

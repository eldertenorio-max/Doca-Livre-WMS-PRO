...
var _WMS_THREE_CDN = 'https://cdn.jsdelivr.net/npm/three@0.160.0';
var _WMS_MAPA3D_VER = '20260604a';

function _wmsLoadScriptOnce(src) {
    return new Promise(function(resolve, reject) {
        var key = (src.split('/').pop() || '').split('?')[0];
        if (key === 'three.min.js' && window.THREE) return resolve();
        if (key === 'OrbitControls.js' && window.THREE && window.THREE.OrbitControls) return resolve();
        if (key.indexOf('wms-mapa-3d.js') === 0 && window.WmsMapa3d) return resolve();

        var existing = null;
        document.querySelectorAll('script[src]').forEach(function(s) {
            if (!existing && (s.getAttribute('src') || '').indexOf(key) >= 0) existing = s;
        });
        if (existing) {
...
function _wmsWithTimeout(promise, ms, msg) {
    return new Promise(function(resolve, reject) {
        var done = false;
        var timer = setTimeout(function() {
            if (done) return;
            done = true;
            reject(new Error(msg || 'Operação expirou'));
        }, ms || 25000);
...
function _wmsCarregarScriptMapa3d() {
    if (window.WmsMapa3d && WmsMapa3d.renderFrame) return Promise.resolve();
    if (_wmsMapa3dScriptPromise) return _wmsMapa3dScriptPromise;
    _wmsMapa3dScriptPromise = _wmsWithTimeout(
        _wmsLoadScriptOnce(_WMS_THREE_CDN + '/build/three.min.js')
            .then(function() {
                if (!window.THREE) throw new Error('Three.js não inicializou');
                return _wmsLoadScriptOnce(_WMS_THREE_CDN + '/examples/js/controls/OrbitControls.js');
            })
            .then(function() {
            ...
            .then(function() {
                if (!window.WmsMapa3d || !WmsMapa3d.renderFrame) {
                    throw new Error('Módulo do mapa 3D não inicializou');
                }
            }),
        30000,
        'Tempo esgotado ao carregar visualização 3D'
    );
...
async function loadWmsMapa3d() {
    ...
    function setLoad(on, msg) {
        if (window.WmsMapa3d && WmsMapa3d.setLoading) WmsMapa3d.setLoading(on, msg);
    ...
    try {
        setLoad(true, 'Carregando dados do armazém…');
        ...
        if (!data.camaras || !data.camaras.length) {
            showMessage('Nenhuma câmara no layout. Verifique data/wms_layout_camaras.json.', 'warning');
            return;
        }
        ...
        await _wmsWithTimeout(
            WmsMapa3d.init({ prefix: 'wms-mapa3d', force: true }),
            20000,
            'Tempo esgotado ao iniciar o renderizador 3D'
        );
        await new Promise(function(r) { requestAnimationFrame(r); });
...
function _wmsEndCellStyle(slot) {
    if (!slot) return { fill: '#cfd8dc', stroke: '#f57c00' };
    var dest = (slot.destino_acao || '').toLowerCase();
    if (dest === 'envio_mg') return { fill: '#42a5f5', stroke: '#f57c00' };
    if (dest === 'retrabalho') return { fill: '#ffca28', stroke: '#f57c00' };
    if (dest === 'descarte_perdas') return { fill: '#8d6e63', stroke: '#f57c00' };
...
function _wmsEndSlotsPorRua(slots, rua) {
    var map = {};
...
function _wmsEndFillRoundGrad(ctx, x, y, w, h, r, top, bot, stroke, sw) {
    _wmsEndRoundRect(ctx, x, y, w, h, r);
...
function _wmsEndDrawFloorGrid(ctx, W, H) {
    ctx.save();
...
function _wmsEndSlotGrad(fill) {
    var m = {
        '#90caf9': ['#e3f2fd', '#64b5f6'],
        '#b3e5fc': ['#e1f5fe', '#4fc3f7'],
        '#c62828': ['#ef5350', '#b71c1c'],
        '#42a5f5': ['#90caf9', '#1e88e5'],
        '#ffca28': ['#fff59d', '#f9a825'],
        '#8d6e63': ['#bcaaa4', '#5d4037'],
        '#78909c': ['#b0bec5', '#455a64'],
        '#ab47bc': ['#e1bee7', '#7b1fa2'],
        '#7e57c2': ['#d1c4e9', '#512da8'],
        '#cfd8dc': ['#eceff1', '#b0bec5']
    };
    return m[fill] || [fill, fill];
}

function _wmsEndDrawSlotCell(ctx, sx, sy, sw, sh, st) {
    if (sw < 2 || sh < 2) return;
    var r = Math.min(2.5, sw / 3, sh / 3);
    var grad = _wmsEndSlotGrad(st.fill);
    _wmsEndFillRoundGrad(ctx, sx, sy, sw, sh, r, grad[0], grad[1], st.stroke, 1.1);
    ctx.save();
    ctx.beginPath();
    _wmsEndRoundRect(ctx, sx + 1, sy + 1, sw - 2, Math.max(1, sh * 0.22), Math.max(0, r - 1));
    ctx.fillStyle = 'rgba(255,255,255,0.35)';
    ctx.fill();
    ctx.restore();
    if (st.fill === '#c62828' && sw > 5 && sh > 4) {
        var pw = sw * 0.68, ph = Math.min(sh * 0.38, 8);
        var px = sx + (sw - pw) / 2, py = sy + sh * 0.28;
        ctx.fillStyle = 'rgba(0,0,0,0.18)';
        _wmsEndRoundRect(ctx, px, py + 1.2, pw, ph, 1);
        ctx.fill();
        var pg = ctx.createLinearGradient(px, py, px, py + ph);
        pg.addColorStop(0, '#8d6e63');
        pg.addColorStop(1, '#4e342e');
        _wmsEndFillRoundGrad(ctx, px, py, pw, ph, 1, '#a1887f', '#5d4037', '#3e2723', 0.6);
    }
}
...
function _wmsEndDrawRackColuna(ctx, x, y, w, h, rua, posMap, maxPos) {
    ...
    ctx.fillStyle = beamG;
    ctx.fillRect(x - 1, y - 5, w + 2, 5);
    ctx.fillRect(x - 1, y + h, w + 2, 4);
...
function _wmsEndDrawAisle(ctx, x, y, w, h, opts) {
    ...
    g.addColorStop(0, isRefrig ? '#cfd8dc' : '#b0bec5');
    g.addColorStop(0.5, isRefrig ? '#eceff1' : '#cfd8dc');
    g.addColorStop(1, isRefrig ? '#cfd8dc' : '#b0bec5');
    ctx.fillStyle = g;
...
function _wmsEndDrawOccBar(ctx, x, y, w, h, pct) {
    _wmsEndRoundRect(ctx, x, y, w, h, h / 2);
    ctx.fillStyle = '#eceff1';
    ctx.fill();
    var fw = Math.max(0, Math.min(w, w * (pct / 100)));
    if (fw > 0) {
        var barG = ctx.createLinearGradient(x, y, x + fw, y);
        var c0 = pct > 85 ? '#e53935' : (pct > 60 ? '#fb8c00' : '#43a047');
        var c1 = pct > 85 ? '#c62828' : (pct > 60 ? '#ef6c00' : '#2e7d32');
        barG.addColorStop(0, c0);
        barG.addColorStop(1, c1);
        ctx.fillStyle = barG;
        _wmsEndRoundRect(ctx, x, y, fw, h, h / 2);
        ctx.fill();
    }
...
function _wmsEndDrawMetalDoor(ctx, x, y, w, h) {
    var dg = ctx.createLinearGradient(x, y, x, y + h);
    dg.addColorStop(0, '#ffd54f');
    dg.addColorStop(0.5, '#ffb300');
    dg.addColorStop(1, '#ff8f00');
    _wmsEndFillRoundGrad(ctx, x, y, w, h, 3, '#ffe082', '#ff8f00', '#f57f17', 1.2);
    var ribs = 5;
    var rw = (w - 6) / ribs;
    ...
    ctx.fillText('ENTRADA', x + w / 2, y + h / 2);
}
...
function _wmsEndDrawCamaraFria(ctx, cam, x, y, bw, bh, regions) {
    ...
    _wmsEndRoundRect(ctx, x, y, bw, bh, 12);
    var cardG = ctx.createLinearGradient(x, y, x, y + bh);
    cardG.addColorStop(0, '#ffffff');
    cardG.addColorStop(1, '#f1f5f9');
    ctx.fillStyle = cardG;
    ctx.fill();
    ctx.shadowColor = 'transparent';
    ctx.strokeStyle = isSelected ? '#1565c0' : '#94a3b8';
    ...
    hdrG.addColorStop(1, isRefrig ? '#01579b' : '#1565c0');
    ctx.fillStyle = hdrG;
    _wmsEndRoundRect(ctx, x + 1, y + 1, bw - 2, headH + padIn - 2, 11);
    ctx.fill();
...
function _wmsEndDrawCamaraCompacta(ctx, cam, x, y, bw, bh, regions) {
    _wmsEndDrawCamaraFria(ctx, cam, x, y, bw, bh, regions);
}

function _wmsEndArea98Style(area) {
    var a = (area || '').toLowerCase();
    if (a.indexOf('envio') >= 0) return { top: '#e3f2fd', bot: '#42a5f5', accent: '#1565c0' };
    if (a.indexOf('retrabalho') >= 0) return { top: '#fff8e1', bot: '#ffca28', accent: '#f57f17' };
    if (a.indexOf('descarte') >= 0) return { top: '#efebe9', bot: '#8d6e63', accent: '#4e342e' };
    if (a.indexOf('avaria') >= 0) return { top: '#f3e5f5', bot: '#ab47bc', accent: '#6a1b9a' };
    if (a.indexOf('reentrega') >= 0) return { top: '#ede7f6', bot: '#7e57c2', accent: '#4527a0' };
    if (a.indexOf('bloqueado') >= 0) return { top: '#eceff1', bot: '#78909c', accent: '#37474f' };
    return { top: '#fffde7', bot: '#ffe082', accent: '#f57c00' };
}

function _wmsEndDrawArea98Zone(ctx, ax, ay, aw, ah, a) {
    var st = _wmsEndArea98Style(a.area || a.label || '');
    _wmsEndFillRoundGrad(ctx, ax, ay, aw, ah, 8, st.top, '#ffffff', st.accent, 1.5);
    ctx.fillStyle = st.accent;
    ctx.fillRect(ax, ay, aw, 4);
    _wmsEndRoundRect(ctx, ax, ay + 4, aw, 14, 0);
    ctx.fillStyle = st.accent;
    ...
    var lbl = (a.label || a.area || '').slice(0, 20);
    ctx.fillText(lbl, ax + 8, ay + 10);
    ctx.font = '8px Segoe UI, sans-serif';
    ctx.fillStyle = '#546e7a';
    ctx.fillText('Rua ' + (a.rua || '—'), ax + 8, ay + 22);
    var slots = a.slots || 10;
    var slotY = ay + 34;
    var slotH = Math.max(ah - 52, 12);
    var sw = Math.min((aw - 16) / Math.max(slots, 1), 18);
    for (var p = 0; p < slots; p++) {
        var status = (a.posicoes && a.posicoes[p]) ? (a.posicoes[p].status || '') : '';
        var fill = status === 'ocupada' ? '#c62828' : '#90caf9';
        _wmsEndDrawSlotCell(ctx, ax + 8 + p * (sw + 3), slotY, sw, slotH, { fill: fill, stroke: '#f57c00' });
    }
    ctx.fillStyle = '#455a64';
...
var _wmsEndVoltar2d = _wmsEndFechar3d;

function _wmsEndAbrirPainel3d() {
    var acc3d = document.getElementById('wms-end-acc-3d');
    if (acc3d) {
        acc3d.open = true;
        acc3d.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    }
}
...
async function wmsEndAbrir3d(opts) {
    ...
    if (selCam) selCam.value = String(cam);
    ...
    try {
        ...
        if (!data.camaras || !data.camaras.length) throw new Error('Nenhuma posição no layout para câmara ' + cam);
...
function _wmsEndBindAccordion() {
    ...
    var btn3d = document.getElementById('btn-wms-end-3d-abrir');
    if (btn3d && !btn3d._wmsBound) {
...
async function loadWmsLocalizacoes() {
    ...
    try {
        ...
        tb.innerHTML = rows.length ? rows.map(function(r) {
            ...
            return '<tr><td>' + cod + '</td><td><strong>' + bcLong + '</strong></td><td>' + escHtml(r.camara) + '</td><td>' + escHtml(r.rua) + '</td><td>' + escHtml(r.posicao) + '</td><td>' + escHtml(r.nivel) + '</td><td><strong>' + escHtml(zl) + '</strong></td><td><strong>' + escHtml(catCol) + '</strong></td><td>' + escHtml(r.status) + '</td><td><button type="button" class="btn btn-sm btn-primary" onclick="wmsImprimirEtqEndereco(\'' + codJs + '\')">ZPL</button> <button type="button" class="btn btn-sm btn-secondary" onclick="wmsPreviaEtqEndereco(\'' + codJs + '\')">HTML</button></td></tr>';
...
async function _wmsBaixarZplUrl(url, nomeArquivo) {
    try {
...
window.wmsPreviaEtqEndereco = function(codigo) {
    if (!codigo) return;
...
window._WMS_BIP_TOTAL_ETAPAS = 2;

var _WMS_BIP_ACOES = {
    1: 'Bipe o EAN/SKU, confirme lote e datas — imprima a etiqueta única e cole no palete.',
...
function wmsAgendarConfirmarDestinoAuto() {
    if (window._wmsBipEtapa !== 2) return;
...
function wmsBipPosProdutoVisivel() {
    var box = document.getElementById('wms-bip-pos-produto');
    return box && box.style.display !== 'none';
}

function wmsBipMostrarPosProduto(show) {
    var box = document.getElementById('wms-bip-pos-produto');
    if (box) box.style.display = show ? 'block' : 'none';
    if (show) wmsBipAtualizarEnderecoEtapa2();
}
...
function wmsBipAtualizarEnderecoEtapa2() {
    ...
    var showAddr = wmsBipPosProdutoVisivel();
    if (!box || !val) return;
    if (fmt) {
        box.style.display = '';
        ...
        if (det) det.textContent = window._wmsSugestaoCarregando
            ...
            : 'Sem indicação automática — libere vagas ou informe manualmente no passo 2.';
    } else {
...
function wmsBipAtualizarResumos() {
    ...
    var prTxt = r.produto ? '✓ ' + r.produto : '';
    if (r.impresso && r.palete) prTxt += (prTxt ? ' · ' : '✓ ') + 'Etiqueta colada (' + r.palete + ')';
    else if (r.palete && wmsBipPosProdutoVisivel()) prTxt += (prTxt ? ' · ' : '') + 'Palete ' + r.palete;
    if (elPr) elPr.textContent = prTxt;
    if (elI) elI.textContent = r.impresso ? '✓ Etiqueta de identificação colada no palete.' : (r.palete ? 'Imprima a etiqueta e cole no palete.' : '');
...
function wmsBipIrParaEtapa(n, opts) {
    opts = opts || {};
    var total = window._WMS_BIP_TOTAL_ETAPAS || 2;
...
function wmsBipFocarEtapa(n) {
    var idMap = { 1: 'wms-pal-sku', 2: 'wms-bip-destino' };
...
function _wmsMostrarSugestao(sug) {
    ...
    if (window._wmsBipEtapa >= 2 && dest && !window._wmsDestinoManual) dest.focus();
    _wmsAplicarModoDestinoSugestao();
    wmsBipAtualizarEnderecoEtapa2();
}
...
async function wmsBipProduto() {
    ...
    if (!skuRaw) {
        wmsMostrarErroBipProduto('Bipe ou digite o EAN/SKU do produto.');
        var skuEl = document.getElementById('wms-pal-sku');
...
async function wmsImprimirEtiqueta(silent) {
    silent = silent === true;
    ...
    if (!etiqueta || etiqueta.length !== 22) {
        if (!silent) showMessage('Confirme o produto antes de imprimir a etiqueta de identificação.', 'error');
        return;
    }
    ...
    try {
        ...
        var w = window.open(url, '_blank');
        if (!w) {
            if (!silent) showMessage('Pop-up bloqueado — libere pop-ups para imprimir.', 'error');
            return;
        }
        wmsBipAtualizarResumos();
        if (!silent) {
            var avisoEnd = fmt ? '' : ' Endereço não calculado — verifique vagas no armazém.';
            showMessage('Etiqueta enviada à impressora. Cole no palete e clique em «Etiqueta colada — ir guardar».' + avisoEnd, fmt ? 'success' : 'warning');
        }
    } finally {
